...
+ (Scanner2 *)scannerWithPage:(CGPDFPageRef)page {
	return [[Scanner2 alloc] initWithPage:page];
}
...
- (FontCollection *)fontCollectionWithPage:(CGPDFPageRef)page {
	...
	if (!dict) 	{
		NSLog(@"Scanner2: fontCollectionWithPage: page dictionary missing");
		return nil;
	}
	
	CGPDFDictionaryRef resources;
	if (!CGPDFDictionaryGetDictionary(dict, "Resources", &resources)) {
		NSLog(@"Scanner2: fontCollectionWithPage: page dictionary missing Resources dictionary");
		return nil;
	}
